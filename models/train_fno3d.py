...
from functions.helpers_deeplearning import get_device, get_batch_size, loss_criterion, RunningAverage, EarlyStopper
from functions.fno_model import FNO_3D
from functions.data_loader import GeologyTracesDataset
...
parser.add_argument('@dv', type=int, default=16, help = "Number of channels after the uplift")
parser.add_argument('@list_dv', type=int, nargs='+', default = [16, 16, 16, 16], help = "Number of channels after each block")
...
if __name__ == '__main__':
    ...
    model = FNO_3D(in_width, d_v, S_in, list_dv, list_D1, list_D2, list_D3, list_M1, list_M2, list_M3, padding=padding)
    ...
    name_config = f"FNO3D-dv{d_v}-S{S_in}-T{T_out}-padding{padding}-learningrate{str(learning_rate).replace('.','p')}-" \
