...
parser.add_argument('@list_dv', type=int, nargs='+', default = [16, 16, 16, 16, 16, 16, 16, 16], help = "Number of channels after each block")
parser.add_argument('@list_D1', type=int, nargs='+', default = [24, 18, 13, 8, 13, 18, 24, 32], help = "Dimensions along the 1st dimension after each block")
parser.add_argument('@list_D2', type=int, nargs='+', default = [24, 18, 13, 8, 13, 18, 24, 32], help = "Dimensions along the 2nd dimension after each block")
parser.add_argument('@list_D3', type=int, nargs='+', default = [24, 18, 13, 8, 17, 34, 64, 320], help = "Dimensions along the 3rd dimension after each block")
parser.add_argument('@list_M1', type=int, nargs='+', default = [12, 9, 6, 4, 4, 6, 9, 12, 12], help = "Number of modes along the 1st dimension after each block")
parser.add_argument('@list_M2', type=int, nargs='+', default = [12, 9, 6, 4, 4, 6, 9, 12, 12], help = "Number of modes along the 2nd dimension after each block")
parser.add_argument('@list_M3', type=int, nargs='+', default = [12, 9, 7, 5, 5, 9, 17, 20, 20], help = "Number of modes along the 3rd dimension after each block")
...
parser.add_argument('@dir_data_val', type=str, nargs='+', default=['inputs3D_S32_Z32_T320_fmax5_val'], help="Name of folders with validation data")
...
if __name__ == '__main__':    
    ...
    list_dv = np.array(options['list_dv'])
    list_D1 = np.array(options['list_D1'])
    list_D2 = np.array(options['list_D2'])
    list_D3 = np.array(options['list_D3'])
    list_M1 = np.array(options['list_M1'])
    list_M2 = np.array(options['list_M2'])
    list_M3 = np.array(options['list_M3'])

    model = UNO_3D(in_width, d_v, S_in, list_dv, list_D1, list_D2, list_D3, list_M1, list_M2, list_M3, padding=padding)
    ...
    name_config = f"UNO3D-dv{d_v}-S{S_in}-T{T_out}-padding{padding}-learningrate{str(learning_rate).replace('.','p')}-" \
